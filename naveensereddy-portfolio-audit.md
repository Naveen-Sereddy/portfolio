...
**Stale screenshots — recaptured via Playwright MCP, `STALE` flags removed:**
The original toolset (Browser-pane `computer` screenshot) has no save-to-disk capability; a native-desktop capture attempt (`claude-in-chrome`'s `save_to_disk`) also produced no file reachable from the local filesystem after an exhaustive search. Playwright MCP's `browser_take_screenshot` does write real files, and was used to recapture all five affected screenshots against the live, fixed product:
- `case-finflow/img/ui-new-expense.png` — recaptured at 1440x900, Employee role, blank form (placeholders visible, no pre-filled data). Matches the FinFlow `5ed450b` fix.
- `case-precisioncab/img/hero-active-pass.png` — recaptured at Cab console (1440) mode, cropped to the `.preview-shell` element (1122x780, native aspect — the original 1440x900 crop included preview-mode chrome that couldn't be reproduced pixel-for-pixel; this crop has zero empty space and no distortion, consistent with Rule 2). No compass sliver, confirms the Precision-Cab `24da0e9` fix live on Vercel.
- `case-testimonium/img/03-conversation-answer.png` (1280x800) and `img/08-low-confidence.png` (1280x800) — recaptured via a real upload (synthetic 10-K generated for this purpose) and two real Gemini queries against testimonium.vercel.app. Badges read "Medium" and "Low" respectively, confirming the Testimonium `91daf6f` fix live in production.
- `case-testimonium/img/06-mobile-conversation.png` (390x844) — a sixth instance of the same stale-label problem, found only by visually inspecting the rendered page after the other four were fixed (it wasn't caught by the earlier filename grep since it's a distinct image file). Recaptured the same way at mobile viewport width.

All five verified rendering correctly in their case-study frames (no distortion, no empty space, consistent sizing with siblings in the same shot-grid) before being committed.
